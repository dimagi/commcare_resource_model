...
if __name__ == '__main__':
    ...
    server_config_model = config = cluster_config_from_path(args.server_config)

    is_excel = bool(args.output)
    if is_excel:
        index = '5'
        writer = ExcelWriter("{}_{}.xlsx".format(args.output, index))
    else:
        writer = ConsoleWriter()

    with writer:

        for server_config_data in all_server_configs:
            server_config_model = server_config_data['model']
            usage = generate_usage_data(server_config_model)

            if args.service:
                server_config_model.services = {
                    args.service: server_config_model.services[args.service]
                }

            service_data = generate_service_data(server_config_model, usage)

            if server_config_model.summary_dates:
                summary_dates = server_config_model.summary_date_vals
            else:
            ...
            summary_data = get_summary_data(server_config_model, service_data)
            for date in summary_dates:
                summaries[date] = summarize_service_data(server_config_model, summary_data, date)
                user_count[date] = usage.loc[date]['users']

            if len(summary_dates) == 1:
                date = summary_dates[0]
                summary_data_snapshot = summaries[date]
                write_summary_data(server_config_model, writer, date, summary_data_snapshot, user_count[date],
                                   field_to_modify=server_config_data["field_to_modify"],
                                   subfield_to_modify=server_config_data["subfield_to_modify"],
                                   step_value=server_config_data["step_value"])
